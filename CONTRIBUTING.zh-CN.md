...
| **`yume-dsl-shiki-highlight`**                                               | 高亮层 — 结构树到彩色 token（本仓库） |
...
```bash
git clone https://github.com/chiba233/yume-dsl-shiki-highlight.git
cd yumeDSL
pnpm install
...
```
fix(shiki-highlight): 修复 tokenizer 对自定义分隔符的处理
```
...
- 保持运行时依赖最小化。本包应聚焦于高亮 / 分词逻辑，避免引入无关依赖。
...
请使用 [Bug 报告](https://github.com/chiba233/yume-dsl-shiki-highlight/issues/new?template=bug_report.yml) 模板，包含：
...
请使用 [功能建议](https://github.com/chiba233/yume-dsl-shiki-highlight/issues/new?template=feature_request.yml) 模板。
